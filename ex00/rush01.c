...
void    rush(int x, int y)
{
        ...
        while (r < y )// check if the row is less than the desire
        {
                ...
                while (c < x)
                {
                        if ((c == 0 && r == 0) || (c == x - 1 && r == y - 1) ) // the corner of the first colum should be "/"
                                write(1, "//", 1);
                        else if((c == 0 && r == y - 1) || (c == x - 1 && r == 0))// the corner of the last colum should be "\"
                                write(1, "\\", 1);
                        else if ((r == 0 || r == y - 1)||(c == 0 || c == x - 1))// to four sides of the rectangle should be "*" except for the coner
                                write(1, "*", 1);
                        else
                                write(1, " ", 1);
                        c++; // to start the loop
